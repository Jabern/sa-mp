...
public OnPlayerLogin(playerid, password[])
{
	...
	if (account)
	{
		...
		{
        	format(str, sizeof str, "|- GM È¨ÏÞ: %d -|", AccountInfo[playerid][AdminLevel]);
...
/*	if(!strcmp(cmd, "/shanche", true)){
		if (AccountInfo[playerid][AdminLevel] >= 3 || IsPlayerAdmin(playerid)){
			if(GetPlayerVehicleID(playerid) && GetPlayerState(playerid)==PLAYER_STATE_DRIVER){
				...
				SendClientMessage(playerid, ORANGE, " ½éÉÜ: /shanche ÒÆ¶¯ÄãËùÓÃµÄ³µÁ¾");
				return true;
			}
		...
		if (AccountInfo[playerid][AdminLevel] >= 5 || IsPlayerAdmin(playerid))
		{
			...
			if(giveplayerid != INVALID_PLAYER_ID)
			{
			    if (level >= 0)
			    {
					...
					format(string, sizeof(string), "[Í¨Öª] ÄúÉý¼¶ %s Îª [È¨ÏÞ:%d] GM", giveplayername,AccountInfo[giveplayerid][AdminLevel]);
	...
	if(!strcmp(cmdtext, "/;", true, 2)) // Admin chat
	{
		if (AccountInfo[playerid][AdminLevel] >= 1 || IsPlayerAdmin(playerid))
		{
		    if(!strlen(cmdtext[2])) return SendClientMessage(playerid, ORANGE, "½éÉÜ: //<text>"), SendClientMessage(playerid, ORANGE, "HELP: Example: //Hi");
			GetPlayerName(playerid, sendername, sizeof(sendername));
		    format(string, sizeof(string), "GM %s[%i]: %s" ,sendername,playerid, cmdtext[2]);
	  		SendClientMessageToAdmins(YELLOW, string, 1);
			return 1;
		}
		else return SendClientMessage(playerid, RED, "You are not an admin!¨q¡É¨r£¨¦á¦ä¦á£©¨q¡É¨r");
	}
		...
		if (AccountInfo[playerid][AdminLevel] >= 2)
		{
		   	...
		   	if(IsPlayerConnected(targetid))
			{
   				if(targetid != INVALID_PLAYER_ID)
       			{
					...
					SendClientMessageToAdmins(ADMIN_RED,string,1);
					format(string, sizeof(string), "-| %s ±»¾¯¸æ´ÎÊý: %d |-",giveplayername, AccountInfo[targetid][Warns]);
					SendClientMessageToAdmins(ADMIN_RED,string,1);
					...
					SendClientMessage(targetid, RED, "3 ´Î¾¯¸æÎÞÐ§ = Auto-Ban!");
					if(AccountInfo[targetid][Warns] >= 3)
					{
					    new ip[128];
						format(string, sizeof(string), "-| GM %s ·âÉ± %s. [Ô­Òò: 3 ´Î¾¯¸æ...] |-", sendername,giveplayername);
						BanLog(string);
						SendClientMessageToAll(ADMIN_RED, string);
						for(new i = 0; i < 3; i++)
						{
						    if (i == 0) format(string, sizeof (string), "-| Ô­Òò: #%d: %s |-", i+1, AccountInfo[targetid][WarnReason1]);
						    else if (i == 1) format(string, sizeof (string), "-| Ô­Òò: #%d: %s |-", i+1, AccountInfo[targetid][WarnReason2]);
						    else if (i == 2) format(string, sizeof (string), "-| Ô­Òò: #%d: %s |-", i+1, AccountInfo[targetid][WarnReason3]);
						    SendClientMessageToAll(ADMIN_RED, string);
						}
	...
	if(strcmp(cmd, "/pstats", true) == 0 || strcmp(cmd, "/zhuangtai", true) == 0) // Views the player's stats
	{
		if(IsPlayerAdmin(playerid) || AccountInfo[playerid][AdminLevel] >= 1)
		{
			...
			if (targetid != INVALID_PLAYER_ID)
			{
	    		...
	    		GetPlayerName(targetid, giveplayername, sizeof(giveplayername));
				format(string, sizeof(string), "%s: ÉúÃü: '%.2f' - ½ðÇ®: '$%d' - ¾¯¸æ´ÎÊý: '%d' - »¤¼×: '%.2f'",giveplayername, health,GetPlayerMoney(targetid),AccountInfo[targetid][Warns],armor);
				SendClientMessage(playerid,PURPLE,string);
				if(AccountInfo[giveplayerid][Logged] == 1) SendClientMessage(playerid, PURPLE, "ÊÇ·ñµÇÂ¼: Yes");
	...
	if(strcmp(cmd,"/acount", true) == 0) // Starts a count down
	{
		...
		if (AccountInfo[playerid][AdminLevel] < 1) return SendClientMessage(playerid,RED,"You're not an admin.¨q¡É¨r£¨¦á¦ä¦á£©¨q¡É¨r");
	...
	{
		if (AccountInfo[playerid][AdminLevel] < 2) return SendClientMessage(playerid,RED,"You're not an admin!¨q¡É¨r£¨¦á¦ä¦á£©¨q¡É¨r");
		tmp = strtok(cmdtext, idx);
		if(!strlen(tmp))
		{
		    ...
		    if (IsPlayerConnected(i))
	  		{
  				if(AccountInfo[i][AdminLevel] >= 1 && AccountInfo[i][AdminLevel] <= 5 && Hide[i] == 0)
	    		{
					...
					format(string, 128, " È¨ÏÞ: %d / [%i]%s", AccountInfo[i][AdminLevel],i,sendername);
					SendClientMessage(playerid, LIGHTGREEN, string);
					count++;
				}
			}
	...
	{
 		new tmppass[128];
		if(AccountInfo[playerid][Logged] == 1) return SendClientMessage(playerid, RED, "    [´íÎó]ÒÑ¾­µÇÂ¼ÁË");
		tmp = strtok(cmdtext, idx);
