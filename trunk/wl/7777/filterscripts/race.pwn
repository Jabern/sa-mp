...
new RRotationDelay = 200000; // How often will we poll for new race start if RRotation is enabled? (Default: 5 minutes, can't be changed IG)
...
new DynaMP=3;           //Dynamic prize multiplier. (Default: 1$/m) [Admin menu ingame]
...
public OnPlayerEnterRaceCheckpoint(playerid)
{
	if(RaceParticipant[playerid]>=1) // See if the player is participating in a race, allows race builders to do their stuff in peace.
	{
		if(RaceParticipant[playerid] == 6) // Player reaches the checkered flag.
	    {
			...
			format(ystring,sizeof(ystring)," 玩家 %s 完成比赛, 获得第 %d 名 ",name,Ranking);
